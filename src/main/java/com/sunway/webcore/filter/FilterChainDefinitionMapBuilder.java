package com.sunway.webcore.filter;

import org.springframework.beans.factory.annotation.Autowired;

import com.sunway.webcore.dto.CoreUrlPermissionDto;
import com.sunway.webcore.dao.UrlPermissionMapper;
...
//        permissionMap.forEach((s, s2) -> {System.out.println(s + ":" + s2);});
    ...
    private LinkedHashMap<String, String> getPermissionDataFromDB(){
        ...
        List<CoreUrlPermissionDto> CoreUrlPermissionDtos = urlPermissionMapper.selectAll();
        if (CoreUrlPermissionDtos != null){
            permissionData = new LinkedHashMap<>(CoreUrlPermissionDtos.size());
            String url;
            String role;
            String permission;
            for (CoreUrlPermissionDto coreUrlPermissionDto : CoreUrlPermissionDtos){
                url = coreUrlPermissionDto.getUrl();
                role = coreUrlPermissionDto.getRole();
